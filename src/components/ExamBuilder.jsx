    ...
    <div className='w-full bg-gray-50 p-6 rounded-2xl flex flex-col gap-6 border border-gray-200 shadow-inner'>
      ...
      <div className='w-full h-14 border-2 bg-white rounded-xl relative overflow-visible'>
        <input 
          ...
          type="text" 
          name="questiontext" // Added name for Playwright
      ...
      <button 
        name="add-option-btn" // Added name for Playwright
        className='h-12 px-6 self-end rounded-xl bg-green-500 text-white font-bold shadow-md hover:bg-green-600 cursor-pointer active:scale-95 transition-all' 
        ...
        <div className='flex flex-row gap-3 items-center w-full' key={e.id}>
          <div className='flex-1 h-14 border-2 bg-white rounded-xl relative overflow-visible'>
            <input 
              ...
              type="text" 
              name={`option-input-${index}`} // Dynamic name for Playwright
              value={e.text} 
          ...
          <button 
            name={`delete-option-${index}`} // Dynamic name for Playwright
            className='h-12 px-4 bg-red-100 text-red-600 rounded-xl font-bold hover:bg-red-200 cursor-pointer' 
            onClick={() => setQuestion(p => ({...p, options: p.options.filter(o => o.id !== e.id)}))}
          >
            Del
          </button>
          <button 
            name={`mark-correct-${index}`} // Dynamic name for the checkbox button
            className={`h-12 w-12 flex items-center justify-center rounded-xl transition-all cursor-pointer ${currentcorrectoption === e.id ? 'bg-green-500 text-white shadow-lg scale-110' : 'bg-gray-200 text-gray-400'}`} 
            onClick={() => { setCurrentCorrectOption(e.id); setShowState(false); }}
          >
            <SiTicktick size={24}/>
          </button>
        </div>
      ...
      {!showstate && (
        <button 
          name="confirm-correct-btn" // Added name for Playwright
          className='h-14 w-full bg-blue-600 text-white font-bold rounded-xl shadow-lg animate-pulse cursor-pointer' 
          onClick={() => { setQuestion(p => ({...p, correctoption: currentcorrectoption})); setShowState(true); }}
        >
      ...
      <button 
        name="add-to-paper-btn" // Added name for Playwright
        className='h-14 w-full bg-gray-800 text-white font-black text-xl rounded-xl disabled:opacity-20 transition-all cursor-pointer' 
        onClick={addQuestion} 
        disabled={!question.correctoption || !question.questiontext}
      >
        Add to Paper
      </button>
    </div>
